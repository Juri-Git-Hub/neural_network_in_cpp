#include "../include/optimizer_rmsprop.hpp"
#include "../include/utils.hpp"

Optimizer_RMSprop::Optimizer_RMSprop(float learning_rate, float decay, float eps, float rho)
    : learning_rate(learning_rate), current_learning_rate(learning_rate),
      decay(decay), iterations(0), eps(eps), rho(rho) {}
...
void Optimizer_RMSprop::update_params(LayerDense &layer) {
    ...
    layer.weight_cache = rho * layer.weight_cache + (1 - rho) * pow(layer.dweights, 2);
    layer.bias_cache = rho * layer.bias_cache + (1 - rho) * pow(layer.dbiases, 2);
